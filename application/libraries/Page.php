...
/**
* Encoding     :   UTF-8
* Created on   :   2015-9-24 14:45:38 by caowenpeng , caowenpeng1990@126.com
*/
...
class Page {
  ...
  /*  public function __construct($total, $pageSize = 10, $pageNumber = 5, $pageParam = 'p') {
    ...
    */
    /**
     * 初始化数据,构造方法
     * @param parmas  所需参数，数组
     *下标为 total 		数据总数量
     *下标为 pageSize 	每页显示条数
     *下标为pageNumber 	分页数字显示数量(使用%numberF%和使用%numberD%会有不同的效果)
     *下标为 pageParam	分页变量
     * @return void
     */
    public function __construct($parmas){

        $this->total = $parmas['total'] < 0 ? 0 : $parmas['total'];
        $this->pageSize =  isset($parmas['pageSize'])? ($parmas['pageSize']< 0  ? 0 : $parmas['pageSize']) : 10 ;
        $this->pageNumber = isset($parmas['pageNumber'])? ($parmas['pageNumber']< 0  ? 0 : $parmas['pageNumber']):10;
        $this->pageParam = isset($parmas['pageParam'])?$parmas['pageParam']:'p';
        $this->calculate();
    }
